...
export function* GetComplaintSaga(action: any): any {
  const { data } = action;
  console.log("GetComplaintSaga action ::", action);

  try {
    const response = yield call(
      ApiCall,
      Method.POST,
      ApiConstants.GET_COMPLAINT_LIST,
      data
    );
    ...
    if (responseJson.response_code === 200) {
      yield put(GetComplaintResponse(responseJson.data));
