...
import { GetBookWishlistResponse } from "../Actions/GetBookWishlist";

export function* GetBookWishlistSaga(action: any): any {
  const { data } = action;
  console.log("GetBookWishlistSaga action ::", action);

  try {
    const response = yield call(
      ApiCall,
      Method.POST,
      ApiConstants.GET_BOOK_WISH_LIST,
      data
    );
    ...
    if (responseJson.response_code === 200) {
      yield put(GetBookWishlistResponse(responseJson.data));
