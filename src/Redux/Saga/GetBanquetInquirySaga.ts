...
import { GetBanquetInquiryResponse } from "../Actions/GetBanquetInquiryAction";

export function* GetBanquetInquirySaga(action: any): any {
  const { data } = action;
  console.log("GetBanquetInquirySaga action ::", action);

  try {
    const response = yield call(
      ApiCall,
      Method.POST,
      ApiConstants.GET_BANQUET_INQUIRY,
      data
    );
    ...
    if (responseJson.response_code === 200) {
      yield put(GetBanquetInquiryResponse(responseJson.data));
