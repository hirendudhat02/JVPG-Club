...
import { GetOfferListResponse } from "../Actions/GetOfferListAction";

export function* GetOfferListSaga(action: any): any {
  const { data } = action;
  console.log("GetOfferListSaga action ::", action);

  try {
    const response = yield call(
      ApiCall,
      Method.POST,
      ApiConstants.GET_OFFER_LIST,
      data
    );
    ...
    if (responseJson.response_code === 200) {
      yield put(GetOfferListResponse(responseJson.data));
