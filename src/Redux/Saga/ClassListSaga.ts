...
import { ClassListResponse } from "../Actions/ClassListAction";

export function* ClassListSaga(action: any): any {
  const { data } = action;
  console.log("ClassListSaga action ::", action);

  try {
    const response = yield call(
      ApiCall,
      Method.POST,
      ApiConstants.GET_CLASS_LIST,
      data
    );
    ...
    if (responseJson.response_code === 200) {
      yield put(ClassListResponse(responseJson.data));
